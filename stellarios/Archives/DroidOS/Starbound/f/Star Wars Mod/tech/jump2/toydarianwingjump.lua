require "/tech/jump2/toydarianjump.lua"
...
function init()
	...
	self.sideSpeed = config.getParameter("sideSpeed",0)
...
function update(args)
	...
	elseif not status.statPositive("activeMovementAbilities") then
		...
		and status.overConsumeResource("energy", self.hoverEnergy) then
			...
			if (lrInput == "left") and (mcontroller.facingDirection() < 0) then
				mcontroller.setXVelocity(mcontroller.facingDirection()*self.sideSpeed+mcontroller.xVelocity())
				--mcontroller.controlModifiers({speedModifier = self.sideSpeed})
			elseif (lrInput == "right") and (mcontroller.facingDirection() > 0) then
				mcontroller.setXVelocity(mcontroller.facingDirection()*self.sideSpeed+mcontroller.xVelocity())
				--mcontroller.controlModifiers({speedModifier = self.sideSpeed})
			end
			self.isJumping = true
